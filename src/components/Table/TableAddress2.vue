<template>
    <el-table 
        ...
        <el-table-column label="状态" >
            <template #default="{row}">
               <template v-if="row.zt == '0'">
                    <el-text type="warning">待审核</el-text>
               </template>
               <template v-if="row.zt == '1'">
                    <el-text type="success">已通过</el-text>
               </template>
               <template v-if="row.zt == '2'">
                    <el-text type="info">已删除</el-text>
               </template>
               <template v-if="row.zt == '3'">
                    <el-text type="danger">已拒绝</el-text>
               </template>
            </template>
        </el-table-column> 
        <el-table-column label="操作" >
            <template #default="{row, $index}">
                <el-popconfirm 
                    title="通过确认" 
                    @confirm="shenhebtn(row.id, 1, $index)"
                ...
                <el-popconfirm 
                    title="拒绝确认" 
                    @confirm="shenhebtn(row.id, 2, $index)"
                    confirm-button-text="确认"
                    cancel-button-text="取消"
                    >
                    <template #reference>
...
const getData = async () => { 
    const res = await $api.address_list({params: paramsObj.value, loading: false}) 
...
async function shenhebtn(id, sh, index) {
    console.log(index)
    const res = await $api.address_shenhe({params: {id, sh}}) 
    if(res.code == 1) {
		ElMessage.success(res.msg)
        // loading.value = true; 
        // await getData()
        // loading.value = false;
        addressList.value[index].zt = sh == '1'? '1' : '3'
    }
